...
export class ContextManager {
   ...
   * @deprecated Use extractCaseNumbers from utils/case-number-extractor directly
   */
   ...
   * Uses resolution-detector utility for improved SRP
   */
  private checkForResolution(context: CaseContext, message: CaseMessage): void {
    const result = detectResolution(message.text, {
    ...
    if (result.isResolved) {
      console.log(
        `[ContextManager] Detected resolution for ${context.caseNumber} from message: "${message.text.substring(0, 100)}" - ${result.reason}`
      );
