...
async function createShortened() {
  const instanceUrl = 'https://mobiz.service-now.com';
  const username = 'SVC.Mobiz.Integration.TableAPI.PROD';
  const password = 'jOH2NgppZwdSY+I';
  const authHeader = 'Basic ' + Buffer.from(username + ':' + password).toString('base64');
  
  const shortName = 'Helpdesk and Endpoint Support - Standard';
  console.log('Trying shorter name:', shortName);
...
createShortened().catch(console.error);
