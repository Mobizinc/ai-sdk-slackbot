...
async function main() {
  ...
  console.log("Testing POST /v1/captures/query with query parameters...\n");

  // Try as POST with query parameters (like GET but using POST method)
  const url = new URL(`${BASE_URL}/v1/captures/query`);
  url.searchParams.set("orgId", ORG_ID!);
  url.searchParams.set("from", String(sevenDaysAgo));
